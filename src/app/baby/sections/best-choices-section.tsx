  ...
  const choices = [
    {
      ...
      description: "Soft, safe, and sustainable basics for baby's delicate skin",
    ...
    <section className="py-20 px-4 bg-gradient-to-b from-white to-[#fdfcfa]">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-[#3A5A40] mb-6">Carefully Curated for Our Little One</h2>
          <p className="text-xl text-[#AE8F65] max-w-3xl mx-auto leading-relaxed">
            Every parent's journey begins with choosing the very best. We've handpicked these exceptional items that
            combine safety, comfort, and style for our precious bundle of joy.
          </p>
        </div>
            ...
            >
              <CardHeader className="p-0">
                <div className="relative overflow-hidden rounded-t-lg">
                  ...
                  />
                  <Badge className="absolute top-4 left-4 bg-[#3A5A40] text-white">{item.badge}</Badge>
                </div>
              </CardHeader>
              <CardContent className="p-6">
                <CardTitle className="text-xl font-bold text-[#3A5A40] mb-3">{item.title}</CardTitle>
                <CardDescription className="text-[#AE8F65] mb-4 text-base">{item.description}</CardDescription>
                <ul className="space-y-2 mb-6">
                  {item.features.map((feature, idx) => (
                    <li key={idx} className="flex items-center text-sm text-[#AE8F65]">
